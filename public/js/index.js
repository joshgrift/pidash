...
window.onload = function () {
  ...
  for (var w in config.widgets) {
    widgets[w] = new Widget(
      new widgetDirectory[config.widgets[w].widget](config.widgets[w].data)
    );
    ...
    html =
      ...
      '<i id="widget-' +
      w +
      '-icon" class="fas fa-x"></i> <span class="small" id="widget-' +
      w +
      '-text"></span>';
  }
...
function loop() {
  ...
  restart_count++;
  if (restart_count == restart_at) location.reload();
